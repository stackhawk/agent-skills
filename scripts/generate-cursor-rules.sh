...
MAPPINGS=(
  ...
  "api/skills/api/SKILL.md|stackhawk-api|Use when querying the StackHawk platform for security reporting, findings analysis, or app management. Triggers include \"stackhawk api\", \"security posture\", \"findings report\", \"show me findings\", \"untriaged findings\", \"which apps\", \"scan history\", \"security dashboard\", \"triage\", \"what needs attention\". Do NOT use for scanning — use the stackhawk-hawkscan rule for \"scan my app\", \"hawkscan\", \"stackhawk.yml\", \"DAST\".||false"
  "api/skills/api/references/api-auth.md|stackhawk-api-auth|StackHawk API authentication reference: JWT token flow, HAWK_API_KEY setup, hawk_api and hawk_api_all_pages helper scripts, token refresh, V1/V2 pagination, 401/403 error diagnosis.||false"
  "api/skills/api/references/api-endpoints.md|stackhawk-api-endpoints|StackHawk API endpoint catalog: REST endpoints for authentication, applications, environments, scan results drill-down chain (scans -> alerts -> findings -> evidence), organization-wide queries, teams, V1/V2 pagination patterns.||false"
  "api/skills/api/references/reporting-recipes.md|stackhawk-api-recipes|StackHawk API reporting recipes: org security posture summary, app deep dive (scan -> alerts -> findings), stale apps detection, scan diff (what changed since last scan). Pre-built jq compositions using hawk_api helpers.||false"
)
