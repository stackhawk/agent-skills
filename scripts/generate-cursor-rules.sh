...
MAPPINGS=(
  ...
  "stackhawk-data-seed/skills/stackhawk-data-seed/SKILL.md|stackhawk-data-seed|Use when the user says \"set up data for HawkScan\", \"my scan has no data to hit\", \"seed this repo for scanning\", or as a first-time-setup step before invoking hawkscan on a fresh repo. Drives the 'hawk perch seed' CLI command, which reads the target repo, designs the minimum seed entities for authenticated scanning, and emits reviewed artifacts under data-seed/ (SQL / HTTP / gRPC / Mongo / shell scripts, a manifest.yaml, and a .data-seed-credentials.env handoff). The seed methodology lives in the hawk CLI; this skill drives the command and relays its JSONL progress. Does NOT write stackhawk.yml (hawkscan owns that). NOT autonomous — explicit user invocation only.|**/data-seed/manifest.yaml,**/.data-seed-credentials.env|false"
)
