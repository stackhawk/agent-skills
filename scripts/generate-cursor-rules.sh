...
MAPPINGS=(
  ...
  "api/skills/api/SKILL.md|stackhawk-api|Use when querying the StackHawk platform for security reporting, findings analysis, or app management. Triggers include \"stackhawk api\", \"security posture\", \"findings report\", \"show me findings\", \"untriaged findings\", \"which apps\", \"scan history\", \"security dashboard\", \"triage\", \"what needs attention\". Prefers the hawkop CLI when installed; falls back to raw REST calls otherwise. Do NOT use for running scans (use the stackhawk-hawkscan rule for \"scan my app\", \"hawkscan\", \"stackhawk.yml\", \"DAST\") or for fixing/remediating code or vulnerabilities — this skill only reads and reports platform data.||false"
  ...
  "stackhawk-data-seed/skills/stackhawk-data-seed/SKILL.md|stackhawk-data-seed|Use when the user says \"set up data for HawkScan\", \"my scan has no data to hit\", \"seed this repo for scanning\", or as a first-time-setup step before invoking hawkscan on a fresh repo. Drives the 'hawk perch seed' subcommands (preflight, validate, finalize): runs the static pre-flight, designs the minimum seed manifest from the repo digest, then validates and finalizes reviewed artifacts under data-seed/ (SQL / HTTP / gRPC / Mongo / shell scripts, a manifest.yaml, and a .data-seed-credentials.env handoff). Requires hawk 5.6.11 or newer; on older hawk it punts with an upgrade instruction. Does NOT write stackhawk.yml (hawkscan owns that). NOT autonomous — explicit user invocation only.|**/data-seed/manifest.yaml,**/.data-seed-credentials.env|false"
  "skill-authoring/skills/skill-authoring/SKILL.md|stackhawk-skill-authoring|Guides authoring and maintaining agent skills in this repository — enforcing Anthropic best practices (SKILL.md body ≤500 lines, reference file ToC, no cross-references between reference files), bumping versions, regenerating Cursor rules, and keeping the plugin structure consistent. Use when editing SKILL.md files, reference files, or plugin manifests; creating a new plugin; or reviewing a skill for best-practice compliance.|**/SKILL.md,**/.claude-plugin/plugin.json,**/.codex-plugin/plugin.json|false"
  ...
  "optimize/skills/optimize/references/cli-contract.md|stackhawk-optimize-cli|Optimize hawkop CLI contract: policy list/get/create/delete/assign and scan metrics commands the skill orchestrates, with flags and JSON output notes. The skill consumes these; it does not recompute metrics.||false"
