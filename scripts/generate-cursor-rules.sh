...
MAPPINGS=(
  ...
  "hawkscan/skills/hawkscan/references/authz-profiles.md|stackhawk-hawkscan-authz-profiles|HawkScan multi-role authorization scanning reference: detecting apps with multiple users/roles (RBAC signals, object-ownership queries, privileged routes), the credential cascade for building 2+ auth profiles (repo fixtures, data-seed multi-user, ask the user, degrade), running scans with --profile-scan-mode=primary-full and --full-scan-profile pinned to the privileged profile, why omitting the mode silently yields a 2-plugin scan, the capability probe before writing profiles, the cost model (one full scan plus N-1 cheap passes, not N times), when policy contents still decide coverage (all-full and offline runs need plugins 422004/422005 enabled), and reading per-profile attribution from the alert Other Info field. Use when an app has multiple roles and BOLA/BFLA coverage is wanted.||false"
