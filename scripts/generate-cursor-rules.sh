...
MAPPINGS=(
  ...
  "bootstrap/skills/bootstrap/SKILL.md|stackhawk-bootstrap|Use when the user says \"set up data for HawkScan\", \"my scan has no data to hit\", \"bootstrap this repo for scanning\", or as a planned first-time-setup step before invoking hawkscan on a fresh repo. Reads a target repo (and any upstream service repos it depends on), proposes the minimum seed entities required for authenticated scanning, dialogs with the user, then emits checked-in artifacts (SQL / HTTP / gRPC / Mongo / shell steps) plus a manifest.yaml and a .bootstrap-credentials.env handoff file. Does NOT write stackhawk.yml (hawkscan owns that), does NOT run the artifacts (deferred to a future Runner), does NOT start the environment. NOT autonomous — explicit user invocation only.|**/bootstrap/manifest.yaml,**/.bootstrap-credentials.env|false"
  "bootstrap/skills/bootstrap/references/discovery.md|stackhawk-bootstrap-discovery|Per-ecosystem storage-type and API-surface detection for the bootstrap skill: PostgreSQL/MySQL/SQLite/Mongo/DynamoDB/Cosmos signals; OpenAPI/protobuf/GraphQL; auth-signal detection; environment config and docker-compose service+port extraction; integration test fixture locations; what to do when discovery is ambiguous; edge cases (monorepos, generated code, vendored deps).||false"
  "bootstrap/skills/bootstrap/references/manifest-schema.md|stackhawk-bootstrap-manifest|Bootstrap manifest schema (Contract B) full reference: top-level structure (version, prerequisites, steps, outputs); step schema with all fields; supported step types (sql, http, grpc, mongo, shell); idempotency primitives (check_sql, check_http, check_command, none); dependency model; self-validation rules; complete falcon example; forbidden patterns.|**/bootstrap/manifest.yaml|false"
  "bootstrap/skills/bootstrap/references/cross-repo-deps.md|stackhawk-bootstrap-cross-repo|Cross-repo upstream service dep resolution for the bootstrap skill: detection signals (docker-compose, env URLs, gRPC stubs, imported clients); resolution flow (sibling-directory search, BOOTSTRAP_REPO_<NAME> env vars, user-confirmation fallback); naming normalization rules; remote-only upstream handling; monorepo handling; confirmation patterns; edge cases.||false"
  "bootstrap/skills/bootstrap/references/idempotency-patterns.md|stackhawk-bootstrap-idempotency|Per-dialect idempotency patterns for bootstrap steps: SQL idioms (Postgres ON CONFLICT, MySQL INSERT IGNORE, SQLite INSERT OR IGNORE); HTTP (PUT over POST); gRPC (Get-then-Create); Mongo (upsert); shell escape hatch conventions; predicate-design rules; anti-patterns to avoid.|**/bootstrap/**/*.sql,**/bootstrap/**/*.http|false"
  "bootstrap/skills/bootstrap/references/falcon-walkthrough.md|stackhawk-bootstrap-walkthrough|End-to-end worked example of the bootstrap skill against ~/dev/falcon: repo profile, cross-repo dep resolution (eyas + yarak), per-upstream discovery, minimal seed proposal, full content of every emitted artifact (manifest.yaml + per-service SQL/HTTP files + README + credentials handoff), manual replay walkthrough, validation criteria.||false"
)
