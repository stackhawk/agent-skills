...
MAPPINGS=(
  ...
  "hawkscan/skills/hawkscan/references/scan-policy.md|stackhawk-hawkscan-scan-policy|HawkScan scan-policy reference: first scan is one broad detected-stack policy run to completion; broadest scan runs last because the last completed scan is the result; follow-up full scans prune tech flags or fix auth/spec instead of raising strength; a named org policy attaches only via app.scanPolicy.name; hand-built policy traps (STRENGTH_LOW/THRESHOLD_LOW dropped as protobuf zero values, stripping pluginType drops passive rules); why optimize Setup runs on every fresh stackhawk.yml.|**/stackhawk.yml,**/stackhawk-*.yml|false"
  "hawkscan/skills/hawkscan/references/input-vectors.md|stackhawk-hawkscan-input-vectors|HawkScan input-vectors reference: active injection rules need a reachable sink; HAR seed (hawk.spider.har) for XML and other non-JSON request bodies because the OpenAPI request builder emits JSON for application/xml operations and blocks XXE; app.openApiConf.customVariables scoped per resource with path regex and the field-name collision caveat; checking a vector was exercised via hawk op scan uris.|**/stackhawk.yml,**/stackhawk-*.yml|false"
