...
endpoints returned empty data (**Step 6**). This skill then consumes its
...
- **Envs group history.** Pick a name deliberately; reuse canonical names
  ...
  before committing. See Step 1c check 6.
...
5. **Does the App exist?** Run `hawkop app list --format json`.
   ...
   - **Multiple name matches:** pick the one whose envs include a host
     matching the URL confirmed in check 1 (if host was established).
   ...
   - **Create path:** run:
     ...
     ```
     Resolve `<env-name>` using the **Env Name Algorithm** above.
...
6. **Does the target Env exist?** Determine the env name using the **Env Name Algorithm** above.
...
Use the `applicationId` and `env` resolved in Step 1c (checks 5–6). Gather
...
1. Step 1a found auth signals but the pattern doesn't match any row in the Phase 1c recipe table — bespoke challenge-response, custom multi-stage flow, client-side computed proof, undocumented scheme, or any case where you'd be guessing which row to pick
...
Before handing findings to the coding agent (Step 6) or the autonomous loop,
...
After Step 5 triage completes, report before the fix loop:
...
→ See `references/false-positives.md` for config-based suppression patterns
  ...
  never enter the triage pipeline. Step 5 handles per-finding triage decisions.
...
2. **Configure:** Before generating or reusing `stackhawk.yml`, verify the
   ...
   tags:
     ...
     - name: _STACKHAWK_GIT_BRANCH
       value: ${BRANCH_NAME}
   ```
3. **Set env vars and validate:** Run the `COMMIT_SHA`, `BRANCH_NAME`, and
   `HAWK_AGENT` export block from **Step 3** ("Validate and Run" → "Pre-flight"),
   then validate:
   ```bash
   timeout 30 hawk validate config stackhawk.yml || echo "Validate timed out — ensure hawk CLI 5.5.0+ is installed (hawk update)"
   ```
...
7. **Report:**
   ...
   - If any findings were filtered by Step 5 triage state, append a
     one-line summary: "Skipped [X] findings already triaged as
     RISK_ACCEPTED / FALSE_POSITIVE."
   - If any findings were marked `FALSE_POSITIVE` in Step 5, summarize:
