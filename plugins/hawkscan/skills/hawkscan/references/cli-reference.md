...
**Option resolution order:** CLI flag → `API_KEY` environment variable → `~/.hawk/hawk.properties`. These docs use `HAWK_API_KEY` as the canonical user-facing name and bridge it per-invocation (`API_KEY=$HAWK_API_KEY hawk ...`).
...
```bash
API_KEY=$HAWK_API_KEY hawk scan          # bridge HAWK_API_KEY into API_KEY for this invocation (alternative to hawk init)
...
API_KEY=$HAWK_API_KEY hawk scan          # or bridge HAWK_API_KEY into API_KEY without init
```
