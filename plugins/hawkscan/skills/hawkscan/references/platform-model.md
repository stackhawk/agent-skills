...
field (`findings[].paths[].status`). The values match the `FindingStatus`
enum in the StackHawk proto:
...
The proto enum values above are the canonical names. JSON serialization
should match exactly (`"status": "NEW"`). If you observe a different
format (e.g., lowercase or title-case) in real output, treat the proto
names as authoritative and file the discrepancy as a platform bug.
...
- **Triage state inherits from the parent scan.** A finding that was
  ...
  scans). Step 4.5's filter applies to rescan results the same way.
