...
3. [The Finding Lifecycle (Triage State Machine)](#the-finding-lifecycle-triage-state-machine)
...
Environments group scans for comparison: StackHawk computes "new", "fixed",
and "reopened" status for findings by comparing successive scans **within
the same env**. A finding that disappears across scans in the Development
env is considered resolved for Development. The same finding in Production
is a separate timeline.
...
## The Finding Lifecycle (Triage State Machine)

Every finding in the JSON output (`hawk scan --json-output` or
`hawkop scan get --detail full --format json`) carries a `triageStatus`.
The states and transitions:

```
        New ───── (human fixes, gone next scan) ─────► (removed)
         │
         ├── (human Accepts in UI)      ──► Accepted  ──┐
         │                                              │
...
| State            | What happened                                                                                | Agent action                                                                             |
|------------------|----------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| `New`            | Finding detected for the first time, not yet triaged                                         | **Fix** in severity order.                                                               |
| `Accepted`       | Human marked "this risk is acceptable" in the platform UI                                    | **Skip.** Re-fixing either wastes effort or overrides a human decision.                  |
| `False Positive` | Human marked "scanner is wrong" in the platform UI                                           | **Skip.** Same reasoning.                                                                |
| `Reopened`       | Was previously `Accepted` / `False Positive` / "fixed and gone"; detected again              | **Prioritize.** Signals a regression or changed context. Fix before same-severity `New`. |

### The agent cannot write triage

There is no API today for marking a finding `Accepted` or `False Positive`
programmatically. If the agent is confident a `New` finding is a true
false positive, the right behavior is to **report** it in the scan
...
- CI run IDs (`ci_run: ${GITHUB_RUN_ID}`)
