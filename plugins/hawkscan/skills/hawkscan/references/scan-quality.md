...
reason identifier used when reporting a gap — SKILL.md's fix-task step consumes these
identifiers directly, so keep them exactly as written here: `coverage-gap`,
...
- **Performance-class signals are never gate gaps.** If health metrics point at slowness
  ...
  reason identifier.
- Keep every reported gap tied to one of the seven reason identifiers above so downstream
  reporting — and the Step 4.5 handoff into fix-task generation — can consume it
