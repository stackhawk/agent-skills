...
description: >
  ...
  "triage", "what needs attention". Do NOT use for scanning — use the hawkscan
  skill for "scan my app", "hawkscan", "stackhawk.yml", "DAST".
---

# StackHawk API Skill

This skill enables Claude to act as a security reporting agent against the StackHawk
platform REST API. The core workflow is:
...
2. **Is `orgId` known?** Required for most endpoints. If unknown:
   - Authenticate (Step 2), then extract it from the login response:
     `jq -r '.organization.id'` on the `/auth/login` response body
...
For reporting workflows that make many API calls, use the reusable helper. It handles
token caching, 401 auto-retry, and full pagination:
...
- Fetch the two most recent `scanId`s for the app from Step 1
