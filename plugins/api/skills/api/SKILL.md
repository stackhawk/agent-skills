...
1. **Is `hawkop` installed and configured?**
   ...
   - If `hawkop` is installed but not configured → run `hawkop init` (interactive) or
     set `HAWKOP_API_KEY` and `HAWKOP_ORG_ID` in the environment.
...
2. **Is `HAWK_API_KEY` set?** Required for the raw API path. If missing and
   `hawkop` is not configured:
   - Direct the user to **app.stackhawk.com → Settings → API Keys**
   - Instruct them to: `export HAWK_API_KEY=hawk.xxxxxxxxxxxx`
   - Do not proceed until one of `hawkop` config or `HAWK_API_KEY` is in place.

3. **Is `orgId` known?** Required for most endpoints.
   - With `hawkop`: `hawkop org get` returns the active org UUID. `hawkop org set <ID>`
...
`hawkop init` once (or `HAWKOP_API_KEY` in headless environments) — the CLI handles token refresh and `401` retry on every subsequent call. No further auth work for this skill.
...
- **Don't hardcode API keys in scripts** — always use `${HAWK_API_KEY}` (raw API) or `${HAWKOP_API_KEY}` (hawkop in CI/CD). Never inline the key value.
