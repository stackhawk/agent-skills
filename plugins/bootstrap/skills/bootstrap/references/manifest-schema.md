...
10. [Complete falcon example](#complete-falcon-example)
...
- id: <stable-id>                   # REQUIRED. Slug (lowercase, hyphens). Used in depends_on.
  ...
  target:                           # REQUIRED. Where to run the step.
    kind: postgres | mysql | sqlite | mongo | http | grpc | none
                                    # REQUIRED. Runner uses this to choose the execution driver.
    connection: <URL or path>       # REQUIRED unless kind is `none`. Env interpolation supported.
  ...
  creates: [ "<kind>:<name>", ... ] # OPTIONAL (recommended). Documents what the step produces.
                                    #   Examples: "org:hawkscan-test-org", "user:hawkscan-test@example.com"
                                    #   v1: documentation only. v2 may enable typed cross-step refs.
  idempotency:                      # REQUIRED. Exactly one primitive (see §6).
    check_sql: "SELECT ..."
    # OR
    check_http:
      method: GET
    ...
    check_command: "..."
    # OR
    none: true                      # Runner refuses unless --allow-non-idempotent is passed.
```

The `target` sub-object uses `connection:` for database-style targets and
...
Referenced file — `bootstrap/eyas/001-orgs.sql`:
```sql
-- Seed test organization for HawkScan scanning.
...
```yaml
- id: falcon-link
  description: Link app to env via falcon REST API
  service: falcon
  type: http
  ...
  depends_on: [ eyas-users, yarak-apps ]
...
Referenced file — `bootstrap/falcon/001-link-app-to-env.http`:
```http
### Link target-app-1 to the default scan environment

PUT {{$env FALCON_URL}}/v1/apps/target-app-1/envs/Development
Content-Type: application/json
Authorization: Bearer {{$env FALCON_ADMIN_TOKEN}}

{
  "orgId": "00000000-0000-0000-0000-000000000001",
  "description": "Default scan environment for HawkScan"
}
```
...
grpcurl \
  ...
  -d '{
    "name":        "hawkscan-test-tenant",
    "displayName": "HawkScan Test Tenant",
...
- id: mongo-users
  ...
  idempotency:
    check_command: >-
      ...
      --eval 'db.users.countDocuments({email:"hawkscan-test@example.com"})' | grep -q '^[1-9]'
```
...
db.users.findOneAndUpdate(
  { email: "hawkscan-test@example.com" },
  {
    $setOnInsert: {
      _id:       "00000000-0000-0000-0000-000000000020",
      email:     "hawkscan-test@example.com",
      password:  process.env.TEST_PASS || "HawkScanTest1!",
      orgId:     "00000000-0000-0000-0000-000000000001",
      role:      "SCAN_USER",
      createdAt: new Date(),
    },
...
- id: apikey-provision
  ...
  creates: [ "apikey:hawkscan-scan-key" ]
  idempotency:
    check_command: "platform-admin apikey list --user hawkscan-test@example.com | grep -q hawkscan-scan-key"
```
...
platform-admin apikey create \
  --user  "hawkscan-test@example.com" \
  --name  "hawkscan-scan-key" \
...
idempotency:
  check_sql: |
    ...
    WHERE u.email = 'hawkscan-test@example.com'
      AND o.name  = 'HawkScan Test Org';
```
...
```yaml
steps:
  - id: eyas-orgs      # no depends_on → depends on nothing (first step)
    ...

  - id: eyas-users     # no depends_on → implicitly depends on eyas-orgs
    ...

  - id: yarak-apps     # no depends_on → implicitly depends on eyas-users
    ...
```
...
steps:
  ...
  - id: eyas-users
    depends_on: [ eyas-orgs ]   # explicit: requires org to exist first
    # ...

  - id: yarak-apps
    depends_on: [ eyas-orgs ]   # explicit: requires org too, but NOT eyas-users
                                #   → parallel-eligible with eyas-users in v2 Runner
    # ...

  - id: falcon-link
    depends_on: [ eyas-users, yarak-apps ]   # requires both to complete first
    # ...
```
...
```yaml
creates: [ "org:hawkscan-test-org", "user:hawkscan-test@example.com" ]
```
...
depends_on: [ "user:hawkscan-test@example.com" ]
```
...
1. **Required step fields present.** Every step has `id`, `type`, `target`,
...
## Complete falcon example

Full manifest for the falcon validation target. This is the reference shape
that Skill A produces for the end-to-end v1 validation scenario.
...
prerequisites:
  ...
  notes: |
    ...
    Ensure EYAS_DB_URL and YARAK_DB_URL are set in your shell or .env file before
    invoking the Runner. See bootstrap/credentials.env.example for a template.
...
steps:
  ...
  - id: falcon-link
    description: Link app to env via falcon REST API (no DB; goes through API gateway)
    service: falcon
    type: http
    file: falcon/001-link-app-to-env.http
    target:
...
VALUES (
  ...
  crypt('HawkScanTest1!', gen_salt('bf')),
  '00000000-0000-0000-0000-000000000001',
  'SCAN_USER',
  NOW()
)
...
**`bootstrap/falcon/001-link-app-to-env.http`**
```http
### Link target-app-1 to the default scan environment
...
- id: eyas-orgs
  ...
  content: |
    ...
    VALUES ('00000000...', 'HawkScan Test Org')
    ON CONFLICT DO NOTHING;
```

**Right:** put the SQL in `eyas/001-orgs.sql` and reference it via `file:`.
...
INSERT INTO users (email, password) VALUES ('hawkscan-test@example.com', 'HawkScanTest1!');
```

**Right:** use env interpolation and store values in `.bootstrap-credentials.env` (gitignored):
```sql
INSERT INTO users (email, password_hash)
VALUES ('hawkscan-test@example.com', crypt(current_setting('app.test_pass'), gen_salt('bf')))
ON CONFLICT (email) DO NOTHING;
```
