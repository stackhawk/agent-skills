...
A hit in a `createTable` block (Liquibase) or a `CREATE TABLE` statement (Flyway) that names the table `users`, `stackhawk_user`, `accounts`, or similar confirms a local password column.
...
|---|---|
| Password column found + BCrypt encoder found | Mark service as "supports password auth". Phase 2 proposes seeding a known test password (default `HawkScanTest1!`). Hash is computed at emission time and embedded in the SQL literal. |
