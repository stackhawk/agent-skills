...
**Do not set `HAWK_AGENT` or `_STACKHAWK_AGENT`.** Those variables attribute scans to the AI
agent that triggered the work; a CI pipeline isn't an agent. `stackhawk.yml` never carries a
`_STACKHAWK_AGENT` tag (the hawk CLI's own detection owns that tag), so there's nothing to
override — leaving these env vars unset lets the CLI's built-in detection run for CI, which is
the correct attribution. Leave it alone.
...
- **Don't set `HAWK_AGENT` or `_STACKHAWK_AGENT`.** CI pipelines aren't agents; `stackhawk.yml` never templates a `_STACKHAWK_AGENT` tag, so leaving these unset lets the hawk CLI's own detection resolve correctly without intervention.
