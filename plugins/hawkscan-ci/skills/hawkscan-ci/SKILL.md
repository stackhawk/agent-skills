...
register/login/mint flows — owned by the `hawkscan` skill, so the same scan
...
- `stackhawk.yml` already has a validated `authentication:` block → add nothing
  to the pipeline; the scan handles auth itself.
- Auth isn't configured, or the scan can't obtain the credential on its own →
  **route the user to the `hawkscan` skill** to add/repair the auth recipe
  (`authentication.script` for register/mint flows), and to `stackhawk-data-seed`
  if the app needs seed *data* to have authenticated routes worth scanning.
- A credential that genuinely cannot live inside the scan (must be seeded
  out-of-band) → only then add a pre-scan step, preferring a committed
  `data-seed/` replay over bespoke scripting.

Don't hand-roll a token-mint step in the workflow: if you're about to, it almost
certainly belongs in the scan's `authentication.script` instead. Full priority
order + ownership table:
[`references/app-startup-patterns.md`](references/app-startup-patterns.md)
Pattern 8.
