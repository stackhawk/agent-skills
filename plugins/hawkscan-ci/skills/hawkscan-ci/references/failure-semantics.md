...
- name: Run HawkScan (warn-only)
  ...
  run: |
    set +e
    docker run --rm ... stackhawk/hawkscan:latest
    EXIT_CODE=$?
    case $EXIT_CODE in
...
GHA's `ubuntu-latest` runners cache popular images automatically. A `stackhawk/hawkscan:latest` pull is ~5 seconds on a warm cache, so an explicit image-cache step is rarely worth it.
...
- name: Resolve current hawk version + binary URL (from the manifest)
  id: hawk
  run: |
    M=https://download.stackhawk.com/hawkdocs/hawk.manifest.json
    echo "version=$(curl -fsSL "$M" | jq -r '.latest.version')" >> "$GITHUB_OUTPUT"
    echo "url=$(curl -fsSL "$M" | jq -r '.latest.assets[] | select(.asset.group=="linux-x64" and (.url|endswith("/hawk"))) | .url')" >> "$GITHUB_OUTPUT"
- name: Cache hawk CLI
...
- **Hard-pinning an old scanner image and leaving it.** A frozen `stackhawk/hawkscan:<old>` misses newly-added checks. Track `:latest` so CI runs current detections; pin a specific version only when you have a reproducibility mandate (and bump it regularly).
