...
**Meaning:** Storage kind `sqlite`. No migration runner — the committed `.sqlite` / `.db` file is the source of truth. This skill notes the file path and advises copy vs. recreate.
...
**Meaning:** Storage kind `dynamo`. No SQL migrations — schema is implicit in CDK constructs or `create_table` calls. This skill surfaces table definitions and emits seed in DynamoDB `PutItem` format.
...
**Meaning:** Storage kind `cosmos`. This skill surfaces container and partition-key definitions from app code or Bicep/ARM templates, and emits seed in Cosmos JSON format.
...
**Meaning:** Storage kind `http`. No seed step. This skill focuses on startup and the upstream dependency map. Upstream deps found in env config (see the Environment Config Detection section below) may have their own storage — trace those repos separately.
...
**Meaning:** Storage kind `grpc` (or `none` if combined with HTTP-only). API surface `grpc`. No SQL seed — the skill emits stub payloads from `.proto` message definitions.
...
These signals tell the skill that end-users must authenticate before
...
When a service is marked "supports password auth", Phase 2 includes the password line in the proposal dialog (see SKILL.md §2.2) and Phase 3.6 emits `TEST_PASSWORD` in `.data-seed-credentials.env` alongside `TEST_PASS`.
...
The result of all detection is an internal service model that feeds Phase 2 of the SKILL.md workflow. Conceptual shape:
...
One `ServiceModel` per service. Monorepos get one model per subdirectory-service. The model is not persisted — if the user re-runs this skill after cloning a new upstream repo, discovery runs again from scratch.
