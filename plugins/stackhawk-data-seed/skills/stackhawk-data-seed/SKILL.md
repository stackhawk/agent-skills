...
1. Pick the smallest set of entities that unblock authenticated, non-trivial routes — typically one org, one user (with credentials), and at least one owned resource (app/project/record) the routes read.
   - **Multi-user shape (when hawkscan asks for authorization/BOLA/BFLA testing):** seed **two peer users at the same privilege level, each owning at least one resource**, plus **one admin/privileged user**. BOLA testing needs peer A to be able to attempt a read of peer B's resource by ID, so two owners with distinct resources are the point — one user with two resources does not test it. BFLA testing needs the low-privilege user plus a privileged surface to attempt. Keep every step idempotent as usual.
...
**Multi-user handoff.** When the multi-user shape was seeded, label each credential pair in `.data-seed-credentials.env` so hawkscan can tell the roles apart and set `isPrivileged` correctly. Use a per-role suffix on the variable names, and make the requirement explicit: the suffix must let hawkscan identify the privileged (admin) pair by text alone (e.g. `_ADMIN`, `_PRIVILEGED`) — never require it to guess from a username, since hawkscan cannot infer privilege that way. Emit one pair per seeded user (two peers plus the admin), and keep the existing single-user variables unchanged so single-profile consumers keep working. Hawkscan discovers these suffixes at read time rather than assuming a fixed name — no shared constant is defined here, since `hawk perch seed finalize` generates the file and a hard-coded name could drift from what it actually emits. Choosing profile names and writing the `profiles:` block stays hawkscan's job.
