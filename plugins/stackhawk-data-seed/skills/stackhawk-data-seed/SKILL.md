...
**Multi-user handoff.** When the multi-user shape was seeded, label each credential pair in `.data-seed-credentials.env` so hawkscan can tell the roles apart and set `isPrivileged` correctly. Use a per-role suffix on the variable names and record which role is privileged — hawkscan cannot infer privilege from a username. Emit one pair per seeded user (two peers plus the admin), and keep the existing single-user variables unchanged so single-profile consumers keep working. Choosing profile names and writing the `profiles:` block stays hawkscan's job.
