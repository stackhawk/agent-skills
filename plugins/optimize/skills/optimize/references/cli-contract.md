...
Notes:
- `policy get` works on **StackHawk presets only**. On an org policy name (anything you
  created, e.g. `OPTIMIZE_TRIAL_…`) it returns `Error: Resource not found` even though
  `policy list` shows it — org policies are only reachable via the org-scoped list. Never
  build a step that depends on reading your own policy back.
