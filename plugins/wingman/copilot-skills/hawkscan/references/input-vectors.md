...
Evidence: hawk 6.4.0 / wingman 2.5.0 (2026-09) against REST fixtures with XML, XPath, and
stored-XSS sinks. Items marked *observed* come from those sessions.
...
- [Checking that a vector was exercised](#checking-that-a-vector-was-exercised)
...
operation) never runs. *Observed:* the XXE rule stayed silent under the spec alone and fired once
a HAR with a real `application/xml` POST was imported.
...
## Checking that a vector was exercised

After the scan, list what was actually requested (skip if the subcommand is unavailable):

```bash
hawk op scan uris <scanId> --format json
```

Look for the XML operation with an *application* status (2xx/4xx), not 500, and for the
parameterized route with the pinned id. A route that only ever returned 404 or 500 was not
tested — fix the vector before reading its silence as a clean result. If the optimize skill is
installed, `hawk op scan metrics <scanId>` shows per-path request health for the same purpose.
