...
`pluginId` is a **string**, quoted, matching the platform's own schema — every entry in a real
`hawk op policy get --name <PRESET>` response is quoted (`"0"`, `"2"`, …). Do not "correct" it
to a bare number; that is a plausible-looking edit that risks failing validation or coercing
oddly, for no gain.
