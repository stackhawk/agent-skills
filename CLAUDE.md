...
- `scripts/generate-marketplace-catalogs.py` — Emits the pinned marketplace catalogs (run by `release.yml` against the marketplace repo checkout)
- `scripts/generate-marketplace-skills.py` — Vendors the released public skills into the marketplace repo's `skills/` for the `skills` CLI (`npx skills add`); run by `release.yml`, tests in `tests/scripts/`
...
- `agent-skills-marketplace/skills/` (in the separate marketplace repo, NOT this repo's `skills/` symlink dir) is generated by `release.yml` via `scripts/generate-marketplace-skills.py` and is never edited by hand — the next release deletes and rebuilds it. It exists because the `skills` CLI (`npx skills add`) reads SKILL.md files only and ignores marketplace.json.
