...
def render_block(matrix: dict) -> str:
    """README badge block: one `### <skill>` section per skill (combos arrive
    pre-sorted by skill, then tool, then model), badges grouped one line per
    tool within a section, each an endpoint image linking to the
    capture-baseline runs page. Assumes matrix["combos"] originates from
    write_outputs - skill/tool/model segments are pre-validated there and used
    verbatim in URLs here."""
