...
def render_block(matrix: dict) -> str:
    """README badge block: one line per tool (combos arrive pre-sorted),
    each badge an endpoint image linking to the capture-baseline runs page.
    Assumes matrix["combos"] originates from write_outputs - tool/model
    segments are pre-validated there and used verbatim in URLs here."""
    ...
    for tool, rows in by_tool.items():
        ...
        for row in rows:
            raw = (f"https://raw.githubusercontent.com/{_REPO}/badges/"
                   f"{row['tool']}/{row['model']}.json")
            img = f"https://img.shields.io/endpoint?url={quote(raw, safe='')}"
            alt = f"{tool} · {display_model(row['model'])}"
