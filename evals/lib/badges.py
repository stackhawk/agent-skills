...
def render_block(matrix: dict) -> str:
    ...
    endpoint image linking to the capture-baseline runs page. Re-validates
    skill/tool/model through _safe_segment before building URLs, so a tampered
    matrix.json read from disk (the --render-readme path) can't emit a
    path-escaping shields URL."""
    ...
    for skill, skill_rows in by_skill.items():
        ...
        for tool, tool_rows in by_tool.items():
            ...
            for row in tool_rows:
                seg_skill = _safe_segment(row["skill"])
                seg_tool = _safe_segment(row["tool"])
                seg_model = _safe_segment(row["model"])
                raw = (f"https://raw.githubusercontent.com/{_REPO}/badges/"
                       f"{seg_skill}/{seg_tool}/{seg_model}.json")
